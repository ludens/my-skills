Title: 서비스 이용 | 서비스 패턴 - 로그인 - KRDS

URL Source: https://www.krds.go.kr/html/site/service/service_03_07.html

Markdown Content:
---
description: 사용자에게 로그인 세션이 만료되기 전에 유지 시간에 제한이 있음을 안내한다.로그인 세션 유지 시간 등의 사유로 인해 서비스 이용 도중 로그아웃되는 상황이 발생할 수 있음을 사용자가 분명히 인지할 수 있도록 안내를 제공한다.
title: 서비스 이용 | 서비스 패턴 - 로그인 - KRDS
...
4. 4.**본문** 모달이 제공되는 상황에 대한 설명, 로그아웃되었을 때의 어려움 등에 대한 설명을 제공함
5. 5.**로그아웃까지 남은 시간** 로그인 세션이 유지되기까지 남은 시간을 표시함
6. 6.**푸터** 모달의 하단 영역으로 로그인 연장, 로그아웃 액션 버튼으로 구성됨
7. 7.**로그인 연장 버튼** 로그인 상태 유지 시간을 연장하기 위한 버튼
