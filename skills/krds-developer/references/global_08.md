Title: 입력폼 | 기본 패턴 - KRDS

URL Source: https://www.krds.go.kr/html/site/global/global_08.html

Markdown Content:
---
description: 입력폼은 사용자가 데이터를 입력하여 서버로 전송하는 데 사용하는 콘텐츠 섹션으로 하나 이상의 입력 컨트롤로 구성된다.
title: 입력폼 | 기본 패턴 - KRDS
...
2. 2.**설명(선택)** 입력폼의 목적에 대한 부가적인 설명이나 사용자가 데이터를 입력할 때 유의해야 하는 사항 등 총체적인 안내를 제공하는 문장
3. 3.**콘텐츠** 데이터 입력을 위한 컴포넌트의 모음
4. 4.**필드셋(선택)** 서로 관련된 여러 데이터 입력 컴포넌트를 의미적으로 그룹화하는 요소
5. 5.**오류 메시지** 사용자의 데이터 입력에 오류가 발생했을 때 제공되는 피드백 메시지
6. 6.**액션 버튼** 입력폼의 데이터를 서버로 전송하거나 입력값 또는 입력 프로세스를 중단하기 위한 액션 버튼 그룹
...
| --------------------- | --------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------- |
| 전입신고                  | 신청안내(전입신고)-step2:조회 전                                                                   | [신청안내(전입신고)-step2:조회 전 html 화면 확인하기 ](../../pattern/sample%5Fappl%5F0304.html "새 창 열기") |
| 신청안내(전입신고)-step2:조회 후 | [신청안내(전입신고)-step2:조회 후 html 화면 확인하기 ](../../pattern/sample%5Fappl%5F0305.html "새 창 열기") |                                                                                         |
| 신청안내(주민등록등본발급)        | [신청안내(주민등록등본발급) html 화면 확인하기 ](../../pattern/sample%5Fappl%5F0302.html "새 창 열기")        |                                                                                         |
| 신청안내(건물대장등본발급)        | [신청안내(건물대장등본발급) html 화면 확인하기 ](../../pattern/sample%5Fappl%5F0301.html "새 창 열기")        |                                                                                         |
| 신청안내(자료신청)            | [신청안내(자료신청) html 화면 확인하기 ](../../pattern/sample%5Fappl%5F0303.html "새 창 열기")            |                                                                                         |
...
__상호작용 가이드라인 클라이언트 측 검증에 대한 표로 구분/설명으로 구성되어있으며 구분 항목은 Keyup, Focusout 로 구성되어있음__
| 구분       | 설명                                                                                                                                                   |
| -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
...
__상호작용 가이드라인 서버 측 검증에 대한 표로 구분/설명으로 구성되어있으며 구분 항목은 Click, Enter 로 구성되어있음__
| 구분    | 설명                                                                                |
| ----- | --------------------------------------------------------------------------------- |
