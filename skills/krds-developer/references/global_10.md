Title: 필터링·정렬 | 기본 패턴 - KRDS

URL Source: https://www.krds.go.kr/html/site/global/global_10.html

Markdown Content:
---
description: 필터링·정렬은 데이터 집합에서 사용자가 원하는 속성과 범주에 속하는 데이터 항목을 선별적으로 표시하거나 특정 속성/범주를 기준으로 조직화하는 방법이다. 필터링·정렬을 이용하면 데이터 목록에서 탐색할 범위를 좁힐 수 있기 때문에 검색 결과 등 목록을 탐색하여 원하는 정보를 찾는 시간을 줄일 수 있다.
title: 필터링·정렬 | 기본 패턴 - KRDS
...
1. 1\. **정렬 컨트롤** 정렬 조건 설정을 위한 입력 양식과 레이블의 모음
2. 2\. **필터 컨트롤** 필터 조건 설정을 위한 입력 양식과 레이블의 모음. 기본으로 노출되어 있거나 상세 필터 표시 버튼을 통해 활성화할 수 있음
3. 3\. **적용된 필터링 조건** 적용된 필터 조건, 조건 수, 조건에 해당하는 데이터 항목 개수, 개별 조건 해제 버튼 등으로 구성됨
4. 4\. **적용된 필터링 조건 해제 버튼** 적용된 모든 필터링 조건을 일괄 해제하기 위한 컨트롤
...
필터링·정렬하고자 하는 데이터 목록 자체 또는 데이터의 열 헤더에 직접 필터링·정렬 컨트롤이 표시된다. 필터링, 정렬 컨트롤을 실행하면 상세 옵션을 위한 목록이 드롭다운에 표시된다. 수평 막대에 비해 수직 공간을 절약할 수 있으나 열 헤더에 컨트롤을 표시할 만큼의 충분한 너비가 확보된 화면에 사용하기 적합하다. 인라인 정렬을 사용할 경우 한 번에 하나의 속성만을 기준으로 정렬할 수 있고 속성 간 관계를 복합적으로 설정할 수 없으므로 필터링·정렬을 사용하는 사용자의 행동이 단순한 경우에 사용하기 적합하다.
...
| ------------------------------------------------------------------------ | -------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| 서비스 신청                                                                   | 타입1                                                                  | [타입1-1 html 화면 확인하기 ](../../pattern/sample%5Fquest%5F01.html "새 창 열기") |
| [타입1-2 html 화면 확인하기 ](../../pattern/sample%5Fquest%5F0101.html "새 창 열기") |                                                                      |                                                                        |
| 타입2                                                                      | [타입2 html 화면 확인하기 ](../../pattern/sample%5Fquest%5F02.html "새 창 열기") |                                                                        |
...
__상호작용 가이드라인 인라인 정렬 정렬 컨트롤 탐색에 대한 표로 구분/설명으로 구성되어있으며 구분 항목은 Tab, Shift + Tab 로 구성되어있음__
| 구분               | 설명                                                |
| ---------------- | ------------------------------------------------- |
...
__상호작용 가이드라인 인라인 정렬 정렬 순서 변경에 대한 표로 구분/설명으로 구성되어있으며 구분 항목은 Click, Enter, Space 로 구성되어있음__
| 구분           | 설명                                                |
| ------------ | ------------------------------------------------- |
