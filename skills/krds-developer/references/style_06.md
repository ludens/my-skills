Title: 아이콘(Icon) | 스타일 가이드 - KRDS

URL Source: https://www.krds.go.kr/html/site/style/style_06.html

Markdown Content:
---
description: 아이콘 가이드는 시스템 아이콘을 중심으로 하며 모든 기관이 동일한 시각 언어와 일관된 사용자 경험을 제공하는 것을 목표로 한다. 브랜드 아이덴티티에 따른 일반 아이콘은 기관마다 다르게 사용할 수 있지만, 시스템 아이콘을 통일적으로 사용함으로써 사용자가 정보를 빠르게 인지하고, 각 기관의 서비스에서 일관된 경험과 신뢰성을 느낄 수 있도록 한다.
title: 아이콘(Icon) | 스타일 가이드 - KRDS
...
브랜드 아이덴티티에 따른 일반 아이콘은 기관마다 다르게 사용할 수 있지만, 시스템 아이콘을 통일적으로 사용함으로써 사용자가 정보를 빠르게 인지하고, 각 기관의 서비스에서 일관된 경험과 신뢰성을 느낄 수 있도록 한다.
...
표준형 스타일을 준수하여 모든 기관에서 동일한 의미와 형태로 시스템 아이콘을 사용하면, 통일된 시각 언어를 통해 사용자가 아이콘을 쉽게 인식하고 기억할 수 있다. 이를 통해 사용자 학습 비용이 줄어들고, 다양한 사이트나 서비스에서도 동일한 의미로 즉각 이해할 수 있어 더 명확하고 일관된 경험을 제공한다.

시스템 아이콘은 일관성, 접근성, 가독성을 최우선으로 고려하여 사용한다.



[ 아이콘 전체 보기 ](https://www.figma.com/@krds "새 창 열림") 

#### 확장자

시스템 아이콘은 일관성 있는 품질과 효율적인 관리를 위해 SVG 파일 형식을 사용한다. SVG는 벡터 기반 확장자로, 화질 저하 없이 다양한 크기로 확대 및 축소가 가능하여 고해상도 화면에서도 선명한 품질을 유지한다. 또한, 파일 크기가 가벼워 로딩 속도를 향상시키고, CSS로 스타일링이 가능해 유연한 디자인 적용이 가능하다.
