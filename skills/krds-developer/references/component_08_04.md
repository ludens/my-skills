Title: 코치마크 (Coach mark) | 컴포넌트 - KRDS

URL Source: https://www.krds.go.kr/html/site/component/component_08_04.html
...
2. 2.**코치마크 팝오버**  
   ...
   2. b.**지시 사항**과업 수행을 위해 사용자가 확인하거나 따라야 할 사항을 안내하는 텍스트  
   3. c.**단계 식별자**전체 단계와 현재 수행 중인 단계 번호를 분수 형식으로 표시함  
   4. d.**액션 버튼**건너뛰기, 이전, 다음 버튼이 제공됨
...
한 페이지에 4개 이하의 코치마크를 사용하는 것이 가장 이상적이며, 최대 15단계를 넘지 않도록 한다. 최적의 코치마크 수를 유지하기 위해 각 코치마크가 사용자에게 도움 되는 정보를 포함하고 있는지 점검해야 한다. 불필요한 코치마크는 삭제하고 가능한 경우 메시지를 합치되, 사용자가 거쳐야 하는 핵심적인 과업 단계가 누락되지 않도록 유의한다.
...
__상호작용 가이드라인 팝오버 내부 콘텐츠 탐색에 대한 표로 구분/설명으로 구성되어있으며 구분 항목은 Tab 로 구성되어있음__
| 구분  | 설명                                         |
| --- | ------------------------------------------ |
...
__상호작용 가이드라인 코치마크 비활성화에 대한 표로 이동에 대한 표로 구분/설명으로 구성되어있으며 구분 항목은 Click, Enter, Space 로 구성되어있음__
| 구분           | 설명                                                                                                  |
| ------------ | --------------------------------------------------------------------------------------------------- |
...
| ------------ | ---------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| 코치마크         | 단계적 실습패널-선택 단계                                                         | [선택 단계 html 화면 확인하기 ](../../pattern/sample%5Fcoach%5F01.html "새 창 열기") |
| 단계적 실습패널-1단계 | [1단계 html 화면 확인하기 ](../../pattern/sample%5Fcoach%5F0101.html "새 창 열기") |                                                                        |
| 단계적 실습패널-2단계 | [2단계 html 화면 확인하기 ](../../pattern/sample%5Fcoach%5F0102.html "새 창 열기") |                                                                        |
| 단계적 실습패널-3단계 | [3단계 html 화면 확인하기 ](../../pattern/sample%5Fcoach%5F0103.html "새 창 열기") |                                                                        |
| 단계적 실습패널-4단계 | [4단계 html 화면 확인하기 ](../../pattern/sample%5Fcoach%5F0104.html "새 창 열기") |                                                                        |

### 마크업 가이드

#### CSS 선택자

__마크업 가이드 CSS 선택자에 대한 표로 필수 로 구성되어있으며 구분 항목은 전체영역, 팝오버로 구성되어있음__
| 필수   |                  |
| ---- | ---------------- |
