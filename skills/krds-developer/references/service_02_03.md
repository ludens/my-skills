Title: 검색어 입력 | 서비스 패턴 - 검색 - KRDS

URL Source: https://www.krds.go.kr/html/site/service/service_02_03.html

Markdown Content:
---
description: 사용자가 검색어 입력 행위에 집중할 수 있도록 한다.헤더에서 확장된 검색어 입력 필드를 사용하는 경우, 검색어 입력 필드와 실시간 검색어 제안 레이어를 제외한 나머지 영역의 명도나 불투명도를 낮추어 사용자가 검색어를 입력하는 행동에 집중할 수 있도록 도와야 한다.
title: 검색어 입력 | 서비스 패턴 - 검색 - KRDS
...
1. 1.**검색어 입력 필드** 사용자가 검색어를 입력하는 텍스트 입력 영역
2. 2.**레이블** 사용자가 검색할 수 있는 내용을 암시하는 유용하고 짧은 텍스트
3. 3.**돋보기 아이콘** 검색을 실행하는 버튼
4. 4.**플레이스홀더(선택)** 검색을 유도하기 위한 문구
5. 5.**검색어 삭제 버튼** 사용자가 검색어 입력 필드에 글자를 입력한 후 제공되어, 실행 시 입력한 검색어 전체를 삭제하는 기능이 작동됨
6. 6.**스코프 필터(선택)** 특정 정보 범주나 섹션에 대한 검색으로 결과를 제한하는 데 사용됨
7. 7.**실시간 검색어 제안(선택)** 검색어 입력 필드가 활성화된 상태에서는 사용자의 최근 검색어, 인기 검색어와 같은 추천 검색어가 제공되며, 검색어 입력 필드에 텍스트 입력이 시작되면 검색어에 기반한 검색어 제안이 제공됨
8. 8.**검색 도움말(선택)** 간단한 검색의 경우 검색어 입력 방식에 대한 예제나 도움말을 직접 제공할 수 있으며, 검색 도움말 문서에 접근할 수 있는 링크로 제공할 수 있음

### 사용성 가이드라인

우수사용자가 검색어 입력 행위에 집중할 수 있도록 한다.
...
권장검색어 입력 필드는 일반적인 검색어가 한눈에 파악될 수 있는 너비로 제공하고 줄바꿈이나 말줄임표를 사용하지 않는다.
...
필수통합 검색이 아닌 검색에는 입력 필드에 레이블을 제공한다.
...
필수플레이스홀더를 사용하는 경우, 유도 문구 외에 검색에 대한 단서나 레이블을 포함해서는 안 된다.
...
권장사용자에게 다양한 방식의 검색어 입력 도움을 제공한다.
...
우수검색어 입력 도움은 가능한 사용자가 검색어를 입력하는 시점에 실시간으로 제공한다.

실시간 검색어 제안은 검색어를 입력하는 시간을 단축함으로써 사용자가 빠르게 검색을 수행할 수 있도록 돕는다.

모범 사례

권장실시간으로 검색어를 제안할 수 없는 경우, 검색 결과 화면에서 보조적인 도움 수단을 제공한다.
...
| ------ | ----------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| 검색     | 통합검색                                                                    | [통합검색 html 화면 확인하기 ](../../pattern/sample%5Fsearch%5F02.html "새 창 열기") |
| 서비스신청  | [서비스신청 html 화면 확인하기 ](../../pattern/sample%5Fappl%5F01.html "새 창 열기")   |                                                                        |
| 정책정보   | [정책정보 html 화면 확인하기 ](../../pattern/sample%5Fquest%5F0101.html "새 창 열기") |                                                                        |
...
__상호작용 가이드라인 검색어 입력 필드에 대한 표로 구분/설명으로 구성되어있으며 구분 항목은 Focusin, Keyup, Enter, 방향키 ↓ 로 구성되어있음__
| 구분      | 설명                                                                                                                                  |
| ------- | ----------------------------------------------------------------------------------------------------------------------------------- |
...
__상호작용 가이드라인 도움말 내부 콘텐츠 탐색에 대한 표로 이동에 대한 표로 구분/설명으로 구성되어있으며 구분 항목은 방향키 ↑, ↓, Click, Enter, Esc 로 구성되어있음__
| 구분       | 설명                                                                                                           |
| -------- | ------------------------------------------------------------------------------------------------------------ |
...
* [버튼](./../component/component%5F05%5F02.html)
* [텍스트 입력 필드](./../component/component%5F09%5F03.html)
