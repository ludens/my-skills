Title: 캐러셀 (Carousel) | 컴포넌트 - KRDS

URL Source: https://www.krds.go.kr/html/site/component/component_04_09.html
...
1. 1.**컨테이너** 캐러셀 항목과 관련 컨트롤이 시각적으로 표시되는 영역
2. 2.**캐러셀 항목**  
 캐러셀을 구성하고 있는 개별 콘텐츠  
   1. a.**기본 항목** 컨테이너 내에서 시각적으로 완전하게 표시되는 항목  
   2. b.**작은/가려진 항목** 항목 목록의 가장 첫 번째, 가장 마지막 항목을 작게 표현하거나 컨테이너 영역에 가려지도록 표현한 것. 숨겨진 콘텐츠가 있음을 암시하여 콘텐츠 탐색을 유도함
3. 3.**탐색 버튼** 항목 목록의 이전/다음 요소를 탐색하는 데 사용되는 버튼
4. 4.**항목 탐색 식별자**  
 캐러셀의 전체 항목 수와 활성화된 항목의 순서 정보를 표시함  
   1. a.**도형** 식별자를 시각적으로 강조하여 표현한 유형으로 일반적으로 원 아이콘이 사용됨. 화면 너비가 충분하거나 전체 슬라이드 수가 5개 이내인 경우에 사용하기 적합하며, 각 슬라이드로 이동할 수 있는 수단으로 활용됨  
   2. b.**분수** 전체 항목 수와 탐색 중인 항목 순서를 숫자로 표시함  
         * 예) 3/5
5. 5.**정지/재생 버튼** 자동 회전 캐러셀에서 움직임을 정지하거나 재생하는 데 사용하는 버튼
...
일정 시간 이후에 다음 슬라이드 항목으로 전환된다. 자동 회전을 사용하는 경우, 항목에 포함된 콘텐츠의 복잡성을 고려하여 적절한 시간 간격으로 콘텐츠가 전환될 수 있도록 해야 한다. 또한 콘텐츠를 이해하는 데 더 많은 시간이 필요한 사용자, 애니메이션에 민감한 사용자를 위해 자동 회전을 정지할 수 있는 컨트롤을 제공해야 한다.
...
작은/가려진 항목에 키보드와 스크린 리더 초점이 접근하게 되면 관련 인터페이스 및 보조 기술 사용자는 수 많은 캐러셀 항목을 모두 탐색하기 전까지 다음 콘텐츠 섹션을 탐색할 수 없다. 작은/가려진 항목에는 tabindex='-1', aria-hidden='true' 속성이나 display:none 스타일을 적용하여 사용자가 의도적으로 항목 탐색을 시도하기 전까지 작은/가려진 항목에 초점이 진입하지 않도록 해야 한다.
...
__상호작용 가이드라인 슬라이드 탐색에 대한 표로 구분/설명으로 구성되어있으며 구분 항목은 Click, Drag & Drop. 방향키 ←, →, Tab, Shift + Tab,Enter, Swipe,Tap 로 구성되어있음__
| 구분          | 설명                                                                                              |
| ----------- | ----------------------------------------------------------------------------------------------- |
...
__상호작용 가이드라인 자동 재생 중지 및 재생에 대한 표로 이동에 대한 표로 구분/설명으로 구성되어있으며 구분 항목은 Tab, Enter, Click, Hover 로 구성되어있음__
| 구분    | 설명                                                                                             |
| ----- | ---------------------------------------------------------------------------------------------- |
...
| ------- | ------------------------------------------------------------------------- | ----------------------------------------------------------------------- |
| 방문      | 방문유형1-1                                                                   | [방문유형1-1 html 화면 확인하기 ](../../pattern/sample%5Fmain%5F01.html "새 창 열기") |
| 방문유형1-2 | [방문유형1-2 html 화면 확인하기 ](../../pattern/sample%5Fmain%5F0101.html "새 창 열기") |                                                                         |
| 방문유형2-1 | [방문유형2-1 html 화면 확인하기 ](../../pattern/sample%5Fmain%5F02.html "새 창 열기")   |                                                                         |
| 방문유형2-2 | [방문유형2-2 html 화면 확인하기 ](../../pattern/sample%5Fmain%5F0201.html "새 창 열기") |                                                                         |
| 방문유형3-1 | [방문유형3-1 html 화면 확인하기 ](../../pattern/sample%5Fmain%5F03.html "새 창 열기")   |                                                                         |
| 방문유형3-2 | [방문유형3-2 html 화면 확인하기 ](../../pattern/sample%5Fmain%5F0301.html "새 창 열기") |                                                                         |

### 마크업 가이드

* KRDS의 캐러셀 컴포넌트는 Swiper 플러그인을 사용하여 제작되었으므로 해당 플러그인에 대한 자세한 사항은 Swiper 플러그인 사이트에서 확인 가능하다.[ Swiper 사이트 바로가기 ](https://swiperjs.com/ "새 창 열림")
* 캐러셀 컴포넌트 사용 시에는  
 /resources/css/plugin/swiper-bundle.min.css  
 /resources/js/plugin/swiper-bundle.min.js  
