Title: 페이지네이션 (Pagination) | 컴포넌트 - KRDS

URL Source: https://www.krds.go.kr/html/site/component/component_03_06.html
...
3. 3.**말줄임표** 번호 링크 사이에 생략된 숫자가 있음을 표시함
4. 4.**현재 화면 식별자** 사용자가 현재 탐색하고 있는 화면을 나타냄

#### 직접 이동



1. 1.**숫자 입력 필드** 탐색하고자 하는 화면 번호를 입력할 수 있는 입력 필드. 입력 전에는 사용자가 현재 보고 있는 화면 번호를 보여줌
2. 2.**전체 화면 수 레이블** 페이지 숫자 입력 필드 오른쪽에 배치되어 전체 화면 수를 보여줌
3. 3.**이동 버튼** 실행 시 숫자 입력 필드에 입력된 숫자에 해당하는 화면으로 이동함

#### 목록 확장



1. 1.**더 보기 버튼** 실행 시 항목을 추가적으로 표시함. '더 보기'라는 텍스트와 표시된 항목 수, 전체 항목 수를 안내하는 분수가 레이블로 제공됨
...
| 변경일자         | 변경 내용                                                                                                        | 리소스                                                                                                                                                            |
| ------------ | ------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2025년 3월 05일 | 페이지네이션 컴포넌트 수정 1페이지 기준 9페이지까지 노출 ⟶ 8페이지까지 노출 이전/다음버튼 비활성화 상태 추가 모바일에서 일부 페이지네이션 버튼 css숨김처리 삭제(작업 시 개발로 처리필요) | [ HTML Component Kit v1.0.4 ](https://github.com/KRDS-uiux/krds-uiux/releases/tag/1.0.4 "새 창 열기")                                                              |
...
__상호작용 가이드라인 숫자 링크 목록에 대한 표로 구분/설명으로 구성되어있으며 구분 항목은 Click, Enter 로 구성되어있음__
| 구분    | 설명                                                             |
| ----- | -------------------------------------------------------------- |
...
__상호작용 가이드라인 페이지 직접 이동에 대한 표로 이동에 대한 표로 구분/설명으로 구성되어있으며 구분 항목은 Click, Space 로 구성되어있음__
| 구분    | 설명                                                                |
| ----- | ----------------------------------------------------------------- |
...
__상호작용 가이드라인 목록 확장에 대한 표로 이동에 대한 표로 구분/설명으로 구성되어있으며 구분 항목은 Click, Space 로 구성되어있음__
| 구분    | 설명                                                                                                                                                     |
| ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
...
__마크업가이드 css 선택자에 대한 표로 필수, 선택항목으로 구성되어있으며 필수항목은 전체영역, (모바일전용) 이전/다음 탐색 그룹,(모바일전용) 페이지 링크 그룹, 이전/다음 탐색, 페이지 링크(선택항목 말줄임표) 로 구성되어있다.__
| 필수       | 선택                                       |      |           |
| -------- | ---------------------------------------- | ---- | --------- |
