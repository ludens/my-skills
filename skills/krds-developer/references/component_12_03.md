Title: 바텀시트 | 컴포넌트 - KRDS

URL Source: https://www.krds.go.kr/html/site/component/component_12_03.html

Markdown Content:
---
description: 바텀 시트는 주요 콘텐츠 또는 선택 옵션을 일시적으로 집중하여 볼 수 있도록 하는 경우 사용한다. 모바일 기기화면의 하단 가장자리에 고정하여 모달 형태로 표현한다.
title: 바텀시트 | 컴포넌트 - KRDS
...
1. 1.**오버레이(Overlay)** 바텀 시트와 기본 창을 시각적으로 구분하기 위한 그림자 또는 가림막
2. 2.**헤더(선택)**바텀 시트에서 제공하는 콘텐츠나 요청하는 행동에 대한 간단한 정보를 담은 제목, 설명을 제공함
3. 3.**핸들(Handle)**정해진 범위 내 값을 선택하는 선택자로, 슬라이더를 따라 좌/우로 이동함
4. 4.**본문**콘텐츠 또는 컴포넌트 요소가 제공되는 영역
5. 5.**닫기 버튼**사용자가 바텀 시트를 닫을 수 있게 하는 버튼 요소로 대게 ‘X’ 아이콘으로 표현됨. 바텀 시트에서 데이터를 선택하여 제출하거나 행동을 확정해야 하는 경우, 닫기 버튼은 바텀 시트를 닫는 동시에 취소와 동일한 기능을 제공함
