Title: 메인 메뉴 (Main menu) | 컴포넌트 - KRDS

URL Source: https://www.krds.go.kr/html/site/component/component_03_02.html

Markdown Content:
---
description: 메인 메뉴는 사용자가 서비스의 정보 구조를 탐색할 때, 가장 많이 사용하는 유형의 메뉴이다. 일반적으로 헤더와 함께 거의 모든 화면에 제공되며, 사용자가 자주 방문하거나 중요도가 높은 화면으로 이동할 수 있는 링크가 포함되어 있다. 빈번하게 활용되는 사용자의 일차적인 이동 수단이기 때문에 사용하기 쉽게 설계해야 한다.
title: 메인 메뉴 (Main menu) | 컴포넌트 - KRDS
...
1. 1.**레이블** 메뉴의 이름으로 하위 수준의 메뉴가 존재하지 않는 경우 링크로 작동하며 하위 수준이 존재하는 경우에는 버튼으로 작동함
2. 2.**꺾쇠 아이콘** 드롭다운 영역의 확장/축소 상태를 표시하는 아이콘
3. 3.**활성화된 메뉴 식별자** 드롭다운 영역이 확장된 메뉴의 변별성을 높이기 위한 시각적 단서
4. 4.**드롭다운 영역** 하위 수준의 메뉴가 제공되는 컨테이너
5. 5.**제목** 드롭다운 영역의 제목으로 최하위 수준 메뉴와 바로 연결된 요소가 제목으로 제공됨
6. 6.**설명** 레이블에 대한 이해를 돕기 위해 제공되는 간단한 설명 텍스트
7. 7.**유틸리티 링크** 활성화된 메뉴와 관련하여 사용자가 자주 찾은 페이지, 도움이 될 만한 정보 및 링크가 제공됨

#### Mobile

1. 1.**햄버거 아이콘** 메뉴 레이어를 활성화하는 데 사용되는 아이콘 버튼
2. 2.**유틸리티 링크** 화면 크기 조정, 언어 선택, 로그인, 자주 사용하는 메뉴 등의 링크가 제공됨
3. 3.**섹션 탐색 버튼** 메뉴 섹션의 구성을 확인하고 탐색하는 데 사용되는 버튼. 정보 구조의 깊이에 따라 레이어 상단에 고정되거나 왼쪽에 고정하여 제공할 수 있음
4. 4.**레이블** 메뉴의 이름으로 링크로 작동함
5. 5.**검색어 입력 필드와 검색 버튼** 메뉴의 수가 많거나 정보 구조의 깊이가 깊은 경우 사용자의 메뉴 탐색을 돕기 위해 사용되는 부분 검색 기능
6. 6.**상위 메뉴 이동 링크** 현재 표시된 하위 수준 메뉴 링크 목록에서 벗어나 기본 메뉴 레이어로 돌아가는 데 사용되는 버튼
...
링크 목록의 수가 많아 뷰포트 높이의 70%를 넘어가는 경우 브라우저 기본 스크롤이 아니라 드롭다운 영역 스크롤을 통해 메뉴를 탐색할 수 있도록 해야 한다. 브라우저 기본 스크롤을 통해 메뉴를 탐색하는 경우, 1수준 링크 레이블을 클릭하거나 배경 마스크를 클릭하여 메뉴를 닫는 과정에서 많은 행동이 요구되며 메뉴를 닫았을 때 본문의 중간부터 콘텐츠를 탐색하게 되는 불편을 겪을 수 있다.
...
__상호작용 가이드라인 메뉴 영역 확장 및 축소 표로 이동에 대한 표로 구분/설명으로 구성되어있으며 구분 항목은 Click, Enter, Esc로 구성되어있음__
| 구분    | 설명                                                                                                      |
| ----- | ------------------------------------------------------------------------------------------------------- |
...
__상호작용 가이드라인 메뉴 링크 탐색에 대한 표로 구분/설명으로 구성되어있으며 구분 항목은 Tab으로 구성되어있음__
| 구분  | 설명                     |
| --- | ---------------------- |
...
<!-- KRDS-CODE:START PC 코드 확인하기 -->
<!-- Source: https://www.krds.go.kr/html/code/main_menu_pc.html -->
```html
<!-- 메인메뉴 : 데스크탑 -->
...
<!-- KRDS-CODE:START Mobile 코드 확인하기 -->
<!-- Source: https://www.krds.go.kr/html/code/main_menu_mobile.html -->
```html
<!-- 메인메뉴 : 모바일 -->
<!-- 이 코드는 샘플용입니다. 실제 동작은 헤더를 참조하세요. -->
  ...
  <div class="gnb-wrap">
    ...
    <div class="gnb-body">
      ...
      <div class="gnb-bottom">
        <a href="#" class="krds-btn small text">메뉴명 <i class="svg-icon ico-angle right"></i></a>
        <a href="#" class="krds-btn small text" target="_blank" title="새 창 열기"> 메뉴명 <i class="svg-icon ico-go"></i></a>
      </div>
      <!-- //gnb-bottom -->
    </div>
    <!-- //gnb-body -->
...
<!-- //메인메뉴 : 모바일 -->
```
<!-- KRDS-CODE:END Mobile 코드 확인하기 -->

| 1Depth | 2Depth                                                             | File Link                                                          |
| ------ | ------------------------------------------------------------------ | ------------------------------------------------------------------ |
| 레이아웃   | 타입1                                                                | [타입1 html 화면 확인하기 ](../../pattern/sample%5Fsub%5F01.html "새 창 열기") |
| 타입2    | [타입2 html 화면 확인하기 ](../../pattern/sample%5Fsub%5F02.html "새 창 열기") |                                                                    |

### 마크업 가이드

#### CSS 선택자 (PC)

__마크업 가이드 CSS 선택자 (PC)에 대한 표로 필수/선택으로 구성되어있으며 필수에 대한 구분 항목은 메인메뉴 전체영역, 메인메뉴 1뎁스 버튼(선택 구분항목 선택된 메뉴,링크 메뉴),메인메뉴 2뎁스 전체영역(선택 구분항목 선택된 메뉴영역),메인메뉴 2뎁스 버튼(선택 구분항목 선택된 메뉴),메인메뉴 3뎁스 전체영역(선택 구분항목 선택된 메뉴영역,신규서비스 배너 우측 배치),메인메뉴 3뎁스 메뉴(선택 구분항목 메뉴 상세설명 있는경우) 로 구성되어있음__
| 필수             | 선택                    |              |                                 |
| -------------- | --------------------- | ------------ | ------------------------------- |
...
| 메인메뉴 3뎁스 메뉴    | .gnb-sub-content > ul | 메뉴 상세설명 있는경우 | .type-description               |

#### CSS 선택자 (MOBILE)

__마크업 가이드 CSS 선택자 (MOBILE)에 대한 표로 필수/선택으로 구성되어있으며 필수에 대한 구분 항목은 헤더 전체영역,유틸리티 영역,로고 및 주요메뉴,메인메뉴 전체영역, 메인메뉴 1뎁스 버튼(선택 구분항목 선택된 메뉴),메인메뉴 2뎁스 전체영역,메인메뉴 2뎁스 버튼,메인메뉴 3뎁스 전체영역,메인메뉴 3뎁스 메뉴(선택 구분항목 4뎁스 있는경우),메인메뉴 4뎁스 메뉴영역(선택 구분항목 선택된 영역) 로 구성되어있음__
| 필수            | 선택                     |          |                                 |
| ------------- | ---------------------- | -------- | ------------------------------- |
...
__마크업 가이드 JS (PC)에 대한 표로 함수/설명으로 구성되어있으며 함수에 대한 구분 항목은 krds\_mainMenuPC(구분항목init(),setupMainTrigger(mainTrigger),setupSubTrigger(subTrigger),toggleMainMenu(mainTrigger),toggleSubMenu(subTrigger),closeMainMenu())로 구성되어있음__
| 함수명                           | 설명                      |                 |
| ----------------------------- | ----------------------- | --------------- |
| krds\_mainMenuPC              | init()                  | 초기화 및 주요 이벤트 실행 |
...
__마크업 가이드 JS (MOBILE)에 대한 표로 함수명/설명으로 함수에 대한 구분 항목은 krds\_mainMenuMobile(구분항목init(),openMainMenu(mobileGnb),closeMainMenu(mobileGnb),setupAnchorScroll(mobileGnb))로 구성되어있음__
| 함수명                          | 설명             |                 |
| ---------------------------- | -------------- | --------------- |
| krds\_mainMenuMobile         | init()         | 초기화 및 주요 이벤트 실행 |
