Title: 완료 | 서비스 패턴 - 신청 - KRDS

URL Source: https://www.krds.go.kr/html/site/service/service_04_07.html

Markdown Content:
---
description: 신청서 제출이 완료된 이후에는 반드시 완료 화면으로 이동하여 사용자가 신청서가 정상적으로 제출되었음을 확인하고 신뢰할 수 있도록 해야 한다. 
title: 완료 | 서비스 패턴 - 신청 - KRDS
...
1. 1.**제목** 완료 화면의 제목 텍스트
2. 2.**확인 정보** 제출 완료된 신청 서식 내용 중 사용자에게 중요한 입력 정보를 요약하여 제공하는 텍스트 목록
3. 3.**액션 컨트롤** 신청 내역 화면 링크, 관련 서비스 링크, 다시 신청하기 버튼 등 신청 완료 이후 사용자의 행동을 유도하기 위한 수단이 제공됨

### 사용성 가이드라인

필수모든 신청 과업에는 완료 단계를 제공한다.
...
권장신청 결과를 확인할 수 있는 화면으로 연결된 링크를 제공한다.
...
우수사용자가 빈번하게 동일한 신청 서비스를 연속적으로 이용하는 경우, 같은 작업을 다시 수행할 수 있는 버튼을 제공한다.
...
권장관련된 신청 서비스 정보를 확인하거나 신청할 수 있는 링크를 제공한다.
