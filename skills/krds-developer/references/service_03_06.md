Title: 로그인 완료 | 서비스 패턴 - 로그인 - KRDS

URL Source: https://www.krds.go.kr/html/site/service/service_03_06.html

Markdown Content:
---
description: 로그인이 완료된 후, 로그인 상황에 적합한 화면으로 이동해야 한다.로그인 플로(Flow)에 진입하기 전의 사용자 여정이 단절되지 않도록 로그인을 시도한 상황에 적합한 화면으로 연결해야 한다.
title: 로그인 완료 | 서비스 패턴 - 로그인 - KRDS
...
4. 4.**로그아웃까지 남은 시간** 로그인 세션이 유지되기까지 남은 시간을 표시함
5. 5.**로그인 연장 버튼** 로그인 상태 유지 시간을 연장하기 위한 버튼

### 사용성 가이드라인

권장로그인이 완료된 후, 로그인 상황에 적합한 화면으로 이동해야 한다.
