Title: 사이드 메뉴 (Side navigation) | 컴포넌트 - KRDS

URL Source: https://www.krds.go.kr/html/site/component/component_03_04.html

Markdown Content:
---
description: 사이드 메뉴는 서브 화면 내에서의 이동을 위해 사용하는 메뉴이다. 일반적으로 본문 영역의 좌측에 사이드바 형태로 제공된다. 메인 메뉴보다 훨씬 좁고 깊은 페이지 구조 탐색에 사용되기 때문에 링크의 개수가 많고 복잡하게 표현되기 쉽다. 사이트 규모가 클수록 사이드 메뉴를 단순하고 직관적으로 설계하여 사용자가 탐색 중에 길을 잃지 않도록 해야 한다.
title: 사이드 메뉴 (Side navigation) | 컴포넌트 - KRDS
...
3. 3.**헤더(선택)** 메뉴 목록의 제목을 제공하는 영역  
   1. a.**제목** 메뉴 목록 전체의 상위 수준 정보 구조를 알려주는 텍스트  
   2. b.**화살표** 제목으로 제공되는 상위 수준 정보 구조로 이동하는 데 사용되는 버튼
4. 4.**기본 화면 링크 목록** 기본 탐색 링크 그룹
5. 5.**하위 수준 페이지 링크 목록** 기본 페이지 링크 하위 수준에 속하는 링크 목록
6. 6.**식별자** 선택된 링크를 변별하기 위한 시각적 단서
7. 7.**꺾쇠 아이콘** 하위 수준 화면 링크 목록의 확장/축소 상태를 나타냄. 사용자는 꺾쇠 아이콘의 유무를 통해 개별 기본 화면 링크 하위에 추가적으로 탐색할 수 있는 링크 목록이 존재하는지를 인지할 수 있음
...
__상호작용 가이드라인 메뉴 링크 탐색에 대한 표로 구분/설명으로 구성되어있으며 구분 항목은 Enter, Click, Tab, Shift + tab 로 구성되어있음__
| 구분               | 설명                           |
| ---------------- | ---------------------------- |
...
__상호작용 가이드라인 메뉴 영역 확장 및 축소에 대한 표로 이동에 대한 표로 구분/설명으로 구성되어있으며 구분 항목은 Click, Enter로 구성되어있음__
| 구분    | 설명                                                                                                                                                                                                        |
| ----- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| 1Depth | File Link                                                           |
| ------ | ------------------------------------------------------------------- |
| 레이아웃   | [레이아웃 html 화면 확인하기 ](../../pattern/sample%5Fsub%5F02.html "새 창 열기") |

### 마크업 가이드

#### CSS 선택자

__마크업 가이드 CSS 선택자에 대한 표로 필수/선택으로 구성되어있으며 구분 항목은 전체영역, 헤더, 기본 목록(1depth), 하위 목록(2depth), 하위 목록(3depth) 등으로 구성되어있음__
| 필수            | 선택                        |        |                   |
| ------------- | ------------------------- | ------ | ----------------- |
...
| 함수명                  | 설명          |                 |
| -------------------- | ----------- | --------------- |
| krds\_sideNavigation | init()      | 초기화 및 주요 이벤트 실행 |
