Title: 네이밍 원칙 | KRDS 소개 - KRDS

URL Source: https://www.krds.go.kr/html/site/utility/utility_03.html

Markdown Content:
---
description: 디자인 시스템의 네이밍 규칙은 시스템 운영 및 관리 측면에서 매우 중요한 역할을 한다. 협업의 효율성을 높이며, 기획자, 디자이너, 개발자들이 작업을 명확히 구분하고 빠르게 이해할 수 있도록 돕는다.
title: 네이밍 원칙 | KRDS 소개 - KRDS
...
토큰에는 띄어쓰기를 사용할 수 없어 토큰에 사용되는 단어 구분은 (\_), (\_\_), (-)등 기호를 사용한다.  
토큰을 관리하는 로컬 배리어블(Local variables)과 이를 연동해서 사용하는 로컬 스타일(Local styles), 코드에서 토큰으로 사용하는 컴포넌트 명은 규칙을 준수하여 사용한다.
...
* 컴포넌트+유형은 컴포넌트 더블언더바(\_\_)유형으로 사용
...
[디자인 토큰 순서 ](../style/style%5F07.html) 
...
light high-contrast 일반 모드 / 선명한 화면 모드
* **속성 (Category)**  
카테고리는 디자인 토큰의 큰 범주로 모든 속성을 포괄할 수 있으며 "color", “typography”, "spacing"처럼 용도를 파악할 수 있도록 사용한다.  
color typography spacing shape 색상 / 글자 / 간격 / 형태
* **컴포넌트 (Component)**  
컴포넌트는 UI 요소를 구성하며, "button", "card"와 같이 일관성 있는 이름으로 정의하여 시스템 내에서 쉽게 파악할 수 있도록 한다.  
button input link card 버튼 / 인풋 / 링크 / 카드
* **유형 (Type)**  
유형은 "background", "surface" 등으로 카테고리 내에서 특정 기능이나 역할을 명시하며, 모든 유형은 일관되게 사용하여 명확성을 높인다.  
background surface icon padding text 배경 / 표면 / 아이콘 / 패딩 / 텍스트
* **변형 (Variant)**  
카테고리의 변형은 "primary", "secondary", “danger”처럼 계층 구조와 시스템적 부분을 고려하여 명명하며, 시각적 변형을 명확히 전달한다.  
primary secondary tertiary 계층 구조  
danger warning success info 시스템
* **요소 (Element)**  
요소는 컴포넌트의 하위 구성 요소로, "label", "body", "text"와 같이 용도에 맞게 명명하여 명확히 구분한다.  
label title body line 레이블 / 타이틀 / 바디 / 라인
* **상태 (State)**  
상태는 "default", "hover"처럼 컴포넌트의 현재 상태를 나타내며, 각 상태는 일관된 네이밍을 사용한다.  
default hover pressed 기본 상태  
selected unselected indeterminate 선택 / 비선택 / 중립
* **크기 (Size)**  
크기는 "small", "large"와 같이 약어가 아닌 명확한 용어로 표현하며, 모든 컴포넌트에 일관되게 적용한다.  
xxsmall xsmall small medium xlarge xxlarge large
* **수정자 (Modifier)**  
수정자는 "rounded", "outlined"처럼 추가적인 스타일이나 변형을 나타내며, 명확한 의미 전달을 위해 간결한 용어를 사용한다.  
rounded square 둥근 / 각진  
line fill 아웃라인 / 채워진  
subtler subtle bold bolder 강조 표현  
lighter light dark darker 밝기 표현

### 토큰 속성 네이밍

전체 토큰 내 속성값은 다르게 표현될 수 있는 요소를 정의하여 일관성 있게 관리하며, 디자인 시스템의 유지보수와 확장성에 기여한다.

__토큰 속성 네이밍에 대한 표로 구분, 설명에 대한 내용으로 구성되어 있으며 구분은 -light, -hight-contrast으로 구성되어있다.__
| 구분               | 설명        |
| ---------------- | --------- |
| \-light          | 기본 모드     |
| \-hight-contrast | 선명한 화면 모드 |

__토큰 속성 네이밍에 대한 표로 구분, 설명에 대한 내용으로 구성되어 있으며 구분은 -background, -surfacet, -divider, -border, -element, -action, -gap, -padding, -fill, -line, -alpha, -neutral, -on(background), -dim(dimmed), -transparency, -inverse, -static 으로 구성되어있다.__
| 구분               | 설명                                                             |
| ---------------- | -------------------------------------------------------------- |
...
__밝기 표현에 대한 표로 구분, 설명에 대한 내용으로 구성되어 있으며 구분은 -lighter, -light, -nomal, -darker 으로 구성되어있다.__
| 구분        | 설명    |
| --------- | ----- |
| \-lighter | 더 밝은  |
| \-light   | 밝은    |
...
__강조 표현에 대한 표로 구분, 설명에 대한 내용으로 구성되어 있으며 구분은 -subtler, -subtle, -bold, -bolder 으로 구성되어있다.__
| 구분        | 설명    |
| --------- | ----- |
| \-subtler | 더 미묘한 |
| \-subtle  | 미묘한   |
...
__강조 표현에 대한 표로 구분, 설명에 대한 내용으로 구성되어 있으며 구분은 -default, -hover, -pressed, -focused, -disabled, -error, -active, -completed, -view, -selected, -unselected, -indeterminate 으로 구성되어있다.__
| 구분              | 설명                    |
| --------------- | --------------------- |
