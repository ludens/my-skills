Title: 모바일 설정 | 기본 패턴 - KRDS

URL Source: https://www.krds.go.kr/html/site/global/global_13.html

Markdown Content:
---
description: 앱 사용 경험 전반을 사용자 주도적으로 제어할 수 있도록 제공하는 관리 허브다. 계정·보안·알림·접근성·언어·데이터·법적 고지 등 정책적/기술적 선택지를 한 곳에 모아, 사용자가 자신의 상황(기기·환경·능력)에 맞게 동작을 조정하고, 위험도가 높은 변경은 충분히 이해·확인하도록 도우며 정부기관 APP의 설정은 공공 서비스 특성상 투명성(무엇이, 왜 바뀌는지)과 복구 가능성(되돌리기/초기화)을 최우선으로 한다.
title: 모바일 설정 | 기본 패턴 - KRDS
...
1. 1.**제목**정보변경이나 제어하려고하는 목적에 대한 설명
2. 2.**다음화면 이동 버튼**정보변경이나 제어의 내용을 많아 다음화면에 표시해야 하는 경우 사용되는 버튼
3. 3.**토글스위치**즉각적인 제어를 목적으로 상태를 전환하는 요소
